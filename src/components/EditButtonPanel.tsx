...
import { Area, Modifier } from '../models/enums';
...
export const EditButtonPanel = ({
    ...
    const [selectedArea, setSelectedArea] = useState<Area>(Area.Main);
    //const layout = config.layouts.find((l) => l.name === layoutName);

    // TODO: how to get groupId for the selected button? Need buttonsToGroup Relationships. Can get layout from layoutName

    // TODO: these useEffects are a bad ideas
    useEffect(() => {
        if (selectedButton) {
    ...
    }, [selectedButton, onGroupFocus]);

    // useEffect(() => {
    //     // when updatedButton changes then run onChange
    //     onChange(updatedButton);
    // }, [updatedButton, onChange]);

    // no button is selected
    if (!selectedButton) {
        return (
            <div>
                <EditPanelHeader panelTitle={panelTitle} onClose={onDiscard} onCreate={onCreate} />
                <div className="edit-form no-select">Select a button or group to modify it</div>
            </div>
        );
    }
        ...
        <div className="edit-panel">
            ...
            <div className="edit-form">
                ...
                <FieldGroup label="PYTHON COMMAND">
                    ...
                    <input
                        type="text"
                        value={updatedButton.command.python ? updatedButton.command.python : ''}
